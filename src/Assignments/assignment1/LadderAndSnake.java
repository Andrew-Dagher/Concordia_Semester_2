...
public class LadderAndSnake {
	...
	protected static int[] player = new int[2];//making array of length 2 to store if player 1 rolls first or player 2
	...
	public void play() {
		...
		for (int i = 0; i < numOfPlayers; i++) {
			Player ply = new Player(player[i]);//make 2 objects that store player number in the next line.
			players[i] = ply;//ply object, that contains player number, gets stored into index i of Player class array. (basically storing the player number into an array of player type)
			}
     	Player.pl=players;//player type arraay stores the whole array of player number
		Board b1=new Board();
		while (true) {
			for(int i = 0; i<numOfPlayers;i++) {
